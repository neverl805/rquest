...
use boring2::ssl::{CertificateCompressionAlgorithm, ExtensionType, SslCurve};
use std::borrow::Cow;
...
pub struct TlsConfig {
    ...
    /// Sets the context's supported curves as a colon-separated string.
    /// This allows specifying curves by name, including experimental ones like X25519MLKEM768.
    /// Takes precedence over `curves` if both are set.
    #[builder(default, setter(strip_option, into))]
    ...
    #[builder(default, setter(transform = |input: impl IntoCertificateCompressionAlgorithm| input.into()))]
    pub cert_compression_algorithm: Option<Cow<'static, [CertificateCompressionAlgorithm]>>,

    /// Sets the context's extension permutation indices.
    #[builder(default, setter(transform = |input: impl IntoExtensionPermutationIndices| input.into()))]
    pub extension_permutation_indices: Option<Cow<'static, [u8]>>,

    /// Sets the context's extension permutation (higher level API using ExtensionType).
    /// This takes precedence over extension_permutation_indices if both are set.
    #[builder(default, setter(transform = |input: impl IntoExtensionPermutation| input.into()))]
...
/// A trait for converting various types into an optional `Cow` containing a slice of `CertificateCompressionAlgorithm`.
///
/// This trait is used to provide a unified way to convert different types
/// into an optional `Cow` containing a slice of `CertificateCompressionAlgorithm`.
pub(crate) trait IntoCertificateCompressionAlgorithm {
    /// Converts the given value into an optional `Cow` containing a slice of `CertificateCompressionAlgorithm`.
    fn into(self) -> Option<Cow<'static, [CertificateCompressionAlgorithm]>>;
}

macro_rules! impl_into_cert_compression_algorithm {
    ($($t:ty => $body:expr),* $(,)?) => {
        $(impl IntoCertificateCompressionAlgorithm for $t {
            fn into(self) -> Option<Cow<'static, [CertificateCompressionAlgorithm]>> {
                $body(self)
            }
        })*
...
impl_into_cert_compression_algorithm!(
    &'static [CertificateCompressionAlgorithm] => |s| Some(Cow::Borrowed(s)),
    Cow<'static, [CertificateCompressionAlgorithm]> => Some,
    &'static CertificateCompressionAlgorithm => |s: &'static CertificateCompressionAlgorithm| Some(Cow::Owned(vec![*s])),
    CertificateCompressionAlgorithm => |s| Some(Cow::Owned(vec![s])),
    Vec<CertificateCompressionAlgorithm> => |s| Some(Cow::Owned(s)),
);

impl<const N: usize> IntoCertificateCompressionAlgorithm for &'static [CertificateCompressionAlgorithm; N] {
    fn into(self) -> Option<Cow<'static, [CertificateCompressionAlgorithm]>> {
        Some(Cow::Borrowed(self))
    }
}

impl<const N: usize> IntoCertificateCompressionAlgorithm for [CertificateCompressionAlgorithm; N] {
    fn into(self) -> Option<Cow<'static, [CertificateCompressionAlgorithm]>> {
        Some(Cow::Owned(self.to_vec()))
    }
}

impl<T: IntoCertificateCompressionAlgorithm> IntoCertificateCompressionAlgorithm for Option<T> {
    fn into(self) -> Option<Cow<'static, [CertificateCompressionAlgorithm]>> {
        self.and_then(|v| v.into())
    }
}

/// A trait for converting various types into an optional `Cow` containing a slice of `u8` for extension permutation indices.
pub(crate) trait IntoExtensionPermutationIndices {
    /// Converts the given value into an optional `Cow` containing a slice of `u8`.
    fn into(self) -> Option<Cow<'static, [u8]>>;
}

macro_rules! impl_into_extension_permutation_indices {
    ($($t:ty => $body:expr),* $(,)?) => {
        $(impl IntoExtensionPermutationIndices for $t {
            fn into(self) -> Option<Cow<'static, [u8]>> {
                $body(self)
            }
        })*
...
impl_into_extension_permutation_indices!(
    &'static [u8] => |s| Some(Cow::Borrowed(s)),
    Cow<'static, [u8]> => Some,
    Vec<u8> => |s| Some(Cow::Owned(s)),
);

impl<const N: usize> IntoExtensionPermutationIndices for &'static [u8; N] {
    fn into(self) -> Option<Cow<'static, [u8]>> {
        Some(Cow::Borrowed(self))
    }
}

impl<const N: usize> IntoExtensionPermutationIndices for [u8; N] {
    fn into(self) -> Option<Cow<'static, [u8]>> {
        Some(Cow::Owned(self.to_vec()))
    }
}

impl<T: IntoExtensionPermutationIndices> IntoExtensionPermutationIndices for Option<T> {
    fn into(self) -> Option<Cow<'static, [u8]>> {
        self.and_then(|v| v.into())
    }
}

/// A trait for converting various types into an optional `Cow` containing a slice of `ExtensionType`.
pub(crate) trait IntoExtensionPermutation {
    /// Converts the given value into an optional `Cow` containing a slice of `ExtensionType`.
    fn into(self) -> Option<Cow<'static, [ExtensionType]>>;
}

macro_rules! impl_into_extension_permutation {
    ($($t:ty => $body:expr),* $(,)?) => {
        $(impl IntoExtensionPermutation for $t {
            fn into(self) -> Option<Cow<'static, [ExtensionType]>> {
                $body(self)
            }
        })*
...
impl_into_extension_permutation!(
    &'static [ExtensionType] => |s| Some(Cow::Borrowed(s)),
    Cow<'static, [ExtensionType]> => Some,
    Vec<ExtensionType> => |s| Some(Cow::Owned(s)),
);

impl<const N: usize> IntoExtensionPermutation for &'static [ExtensionType; N] {
    fn into(self) -> Option<Cow<'static, [ExtensionType]>> {
        Some(Cow::Borrowed(self))
    }
}

impl<const N: usize> IntoExtensionPermutation for [ExtensionType; N] {
    fn into(self) -> Option<Cow<'static, [ExtensionType]>> {
        Some(Cow::Owned(self.to_vec()))
    }
}

impl<T: IntoExtensionPermutation> IntoExtensionPermutation for Option<T> {
    fn into(self) -> Option<Cow<'static, [ExtensionType]>> {
        self.and_then(|v| v.into())
    }
}
