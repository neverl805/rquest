...
impl TlsConnector {
    ...
    pub fn new(config: TlsConfig) -> crate::Result<TlsConnector> {
        ...
        // Set OCSP stapling
        set_bool!(config, enable_ocsp_stapling, connector, enable_ocsp_stapling);

        // Set Signed Certificate Timestamps
        set_bool!(
            config,
        ...
        // Set permute extensions
        set_option!(config, permute_extensions, connector, set_permute_extensions);

        // Set extension permutation (higher priority than extension_permutation_indices)
        if let Some(ref permutation) = config.extension_permutation {
            connector.set_extension_permutation(permutation.as_ref())?;
        }

        // Set curves list (higher priority than curves)
        if let Some(curves_list) = config.curves_list.as_deref() {
            connector.set_curves_list(curves_list)?;
        } else if let Some(curves) = config.curves.as_deref() {
        ...
        set_option_ref_try!(config, delegated_credentials, connector, set_delegated_credentials);

        // Set preserve TLS 1.3 cipher list (MUST be before set_cipher_list)
        set_option!(
            config,
        ...
        if let Some(cert_compression_algorithm) = config.cert_compression_algorithm {
            for algorithm in cert_compression_algorithm.iter() {
                connector = connector.add_cert_compression_algorithmorithm(*algorithm)?;
            }
        }
