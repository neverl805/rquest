...
use crate::tls::conn::cert_compression::{
    BrotliCertificateCompressor, ZlibCertificateCompressor, ZstdCertificateCompressor,
};
use boring2::{
    error::ErrorStack,
    ssl::{
        CertificateCompressionAlgorithm, ConnectConfiguration, SslConnectorBuilder, SslOptions, SslRef,
        SslVerifyMode,
    },
};
use std::borrow::Cow;
...
pub trait SslConnectorBuilderExt {
    ...
    fn add_cert_compression_algorithmorithm(
        self,
...
impl SslConnectorBuilderExt for SslConnectorBuilder {
    ...
    fn add_cert_compression_algorithmorithm(
        ...
        res.map(|_| self).map_err(Into::into)
    }
